...
def _build_question_prompt(
    ...
    # ── Финальное напоминание (sandwich) ────────────────────────────────
    if sec_description:
        lines += [
            "",
...
def _call_ollama(prompt: str, system: str = "") -> str:
    ...
    response = ollama.chat(
        model="qwen2.5:7b",
        messages=messages,
        options={"temperature": 0.7, "top_p": 0.9, "num_predict": 3000},
    )
...
def _call_llm(prompt: str, provider: str = "ollama", api_key: str = "", system: str = "") -> str:
...
def generate_analysis_docx(questions: list, progress_callback=None, provider: str = "ollama", api_key: str = "") -> bytes:
    ...
    for idx, q in enumerate(questions, start=1):
        ...
        try:
            ...
            analysis = _call_llm(user_prompt, provider=provider, api_key=api_key, system=system_prompt)
